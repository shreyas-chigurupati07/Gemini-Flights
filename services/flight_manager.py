...
def book_flight(flight_id: int, seat_type: str, num_seats: int = 1, db: Session = Depends(get_db)):
    """
    Handles the booking of a flight for a user based on various criteria. It retrieves the flight from the database,
    checks if enough seats are available, and updates the database accordingly.

    Parameters:
    - flight_id (int): The ID of the flight to book.
    - seat_type (str): The type of seat to book (economy, business, or first_class).
    - num_seats (int, optional): The number of seats to book (default is 1).
    - db (Session, default Depends(get_db)): SQLAlchemy database session for executing queries.

    Returns:
    - A dictionary containing a success message and flight information.
    """
    # Retrieve the flight from the database
    flight = db.query(Flight).filter(Flight.flight_id == flight_id).first()
    ...
    # Return a success message
    return {"message": f"Successfully booked {num_seats} {seat_type} seat(s) on {flight.airline} flight on {flight.departure_date} from {flight.origin} to {flight.destination}. Total cost: ${total_cost}."}
